// app/base/guardado/local/page.tsx
'use client';
...
const TITULO = 'Guardado Local';

export default function GuardadoLocalPage() {
  return (
    <Suspense
      fallback={
        <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-violet-800 via-fuchsia-700 to-indigo-800 text-white">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-sm opacity-80">
              Cargando pedidos guardados (local)…
            </span>
          </div>
        </main>
      }
...
function GuardadoPageInner() {
    ...
    (async () => {
      try {
        ...
        const { data: rows, error: e1 } = await supabase
          ...
          .eq('estado', 'GUARDADO')
          // LOCAL = tipo_entrega LOCAL o NULL
  ...
  async function sendComprobanteLink(p?: Pedido | null) {
    ...
    try {
      ...
      const texto = [
        `*SERVICIO N° ${p.id}*`,
        'Ya está listo, puedes revisarlo aquí.:',
        link,
        '',
      ...
      <section className="relative z-10 w-full px-3 sm:px-6 lg:px-10 grid gap-4 mt-2">
        ...
        {!loading && !errMsg && pedidos.length === 0 && (
          <div className="mt-6 text-white/80">
            No hay pedidos en estado GUARDADO (LOCAL).
          </div>
        )}
