...
const ESTADOS: PedidoEstado[] = ['LAVAR', 'LAVANDO', 'GUARDAR', 'GUARDADO', 'ENTREGADO', 'ENTREGAR'];

const LABEL_ESTADO: Record<PedidoEstado, string> = {
  LAVAR: 'Lavar',
  LAVANDO: 'Lavando',
  GUARDAR: 'Guardar',
  GUARDADO: 'Guardado',
...
export default function GeneralPage() {
  ...
  async function changeEstado(id: number, next: PedidoEstado) {
    ...
    if (next !== estadoFiltro) {
      ...
      snack(`Pedido #${id} actualizado en estado ${next}`);
    }
    setSaving(false);
  }
  ...
  const labelEstado = LABEL_ESTADO[estadoFiltro];
          ...
          {!loading &&
            ...
            (Array.isArray(pedidos) ? pedidos.length === 0 : true) && (
              <div className="text-white/80">
                No hay pedidos en estado {labelEstado.toUpperCase()}.
              </div>
            )}
