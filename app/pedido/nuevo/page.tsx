...
export default function NuevoPedidoOneStep() {
  const router = useRouter();

  // ---- UI & data state
  const [tel, setTel] = useState('');
  const [cliente, setCliente] = useState<Cliente | null>(null);
  const [needCreate, setNeedCreate] = useState(false); // abre modal si no existe
  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await supabase.from('articulo').select('id,nombre,valor').order('nombre');
      if (!error && data) setArticulos(data as Articulo[]);
    })();
  }, []);

  // ---------- Buscar cliente (autosubmit on 9 dígitos)
  useEffect(() => {
  ...
  async function lookupCliente(tlf: string) {
    try {
      ...
      setErr(null);
      const { data, error } = await supabase.from('cliente').select('*').eq('telefono', tlf).maybeSingle();
      ...
      if (data) {
        ...
        // No existe → mostrar modal de creación
        setCliente(null);
        setNeedCreate(true);
        setModal({ open: true });
      }
  ...
  // ---------- Nro correlativo (RPC o fallback)
  async function getNextNumber(): Promise<NextNumber> {
    ...
    // fallback: max(id)+1
    const { data: d2, error: e2 } = await supabase.from('pedido').select('id', { count: 'exact', head: false });
    if (e2) {
      const now = new Date();
      return { nro: Number(now.getTime().toString().slice(-6)), fecha: now.toISOString().slice(0, 10), entrega: now.toISOString().slice(0, 10) };
    }
    const maxId = Array.isArray(d2)
      ? (d2 as any[]).reduce((m, r) => (typeof r.id === 'number' && r.id > m ? r.id : m), 0)
      : 0;
    const now = new Date();
    return { nro: maxId + 1, fecha: now.toISOString().slice(0, 10), entrega: now.toISOString().slice(0, 10) };
  }

  // ---------- Subir foto (opcional) y obtener URL
  ...
  async function guardarPedido() {
    ...
    try {
      ...
      // Insert simple con JSON de items (ajusta a tu esquema si usas tabla detalle)
      ...
      // Ir a detalle/impresión si quieres, o a base
      router.push(`/base`);
    } catch (e: any) {
  ...
  return (
    <main className="relative min-h-screen text-white bg-gradient-to-br from-violet-800 via-fuchsia-700 to-indigo-800 pb-24">
      ...
      <section className="relative z-10 mx-auto max-w-5xl px-4">
        {/* BLOQUE ÚNICO: TELÉFONO + CLIENTE + ARTÍCULOS + TABLA + FOTO + GUARDAR */}
